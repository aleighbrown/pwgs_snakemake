...
def populations_on_tree(summary_file, tree):
    with open(summary_file, "r") as read_file:
        data_summary = json.load(read_file)
    pop_prev_dict = {}
    for p in data_summary['trees'][tree]['populations']:
        pop_prev_dict[p] = data_summary['trees'][tree]['populations'][p]['cellular_prevalence']
    
    return(pop_prev_dict)
...
def read_best_trees_json(mutasgn_path, summary_file, k0 = 5):
    ...
    for tree in best_trees:
        ...
        #get the cellular prevalence of the clones in those trees
        #cell_prev = populations_on_tree(summary_file, tree)
        #tree_df = tree_df['population'].map(cell_prev)
