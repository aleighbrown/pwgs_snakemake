  ...
  def _calc_read_counts(self, variant):
    ...
    if alt is None:
      ...
      try:
        ...
        variant_reads = int(getattr(variant.samples[tumor_i].data, 'TIR')[0])

    ref_reads = total_reads - variant_reads
    return (ref_reads, total_reads)

class StrelkaCombinedParser(VariantParser):
  def __init__(self, vcf_filename, tumor_sample=None):
    self._vcf_filename = vcf_filename
    self._tumor_sample = tumor_sample
...
class CnvFormatter(object):
  ...
  def _format_cnvs(self, cnvs, variants):
    print('Estimated read depth: %s' % self._read_depth)
      ...
      while idx < len(positions):
        ...
        for pos, pt_type, (sampidx, cell_prev, major_cn, minor_cn) in points_at_locus:
          if pt_type == 'start':
            ...
            print('Removing ', (pos, pt_type, sampidx, cell_prev, major_cn, minor_cn))
...
def parse_variants(samples, vcf_files, vcf_types, tumor_sample, missing_variant_confidence):
  ...
  for sample in samples:
    ...
    elif vcf_type == 'strelka_indelcombined':
      variant_parser = StrelkaCombinedParser(vcf_fn, tumor_sample)
    else:
...
def main():
  all_vcf_types = set(('sanger', 'mutect_pcawg', 'mutect_smchet', 'mutect_tcga', 'muse','dkfz', 'strelka', 'vardict', 'pcawg_consensus', 'strelka_indelcombined'))
